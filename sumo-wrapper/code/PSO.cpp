...
int main(int argc, char **argv)
{
  ...
  string outputFile = c.getPath() + settings.dir + "/" + settings.outputFile;
	...
	command = "code/sumo-wrapper " + instance + " " + settings.dir + " " + inputFile + " " + outputFile + " 1";
...
//	initial_time = 1000000*tv.tv_sec + tv.tv_usec;
    ...
    // Only prints the best solution
		/*for(int i = 0; i < settings.dimension; i++)
			cout << algorithm.best[i] << " ";
		cout << endl;*/

    // Prints all solutions 
    cout << "Current swarm:" << endl;
	  for(unsigned i = 0; i < settings.swarmsize; i++) {
		 for(unsigned j = 0; j < settings.dimension; j++)
		   cout << algorithm.swarm[i][j] << " ";
     cout << algorithm.swarm_fit[i] << endl;
    }

    cout << "Best positions:" << endl;
	  for(unsigned i = 0; i < settings.swarmsize; i++) {
		 for(unsigned j = 0; j < settings.dimension; j++)
		   cout << algorithm.best_pos[i][j] << " ";
     cout << algorithm.best_pos_fit[i] << endl;
    }
